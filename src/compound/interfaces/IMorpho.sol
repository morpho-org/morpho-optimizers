...
interface IMorpho {
    ...
    function defaultMaxGasForMatching() external view returns (uint64 supply, uint64 borrow, uint64 withdraw, uint64 repay);
    function maxSortedUsers() external view returns (uint256);
    function dustThreshold() external view returns (uint256);
    function supplyBalanceInOf(address, address) external view returns (uint256 inP2P, uint256 onPool);
    function borrowBalanceInOf(address, address) external view returns (uint256 inP2P, uint256 onPool);
    function enteredMarkets(address, uint256) external view returns (address);
    function deltas(address) external view returns (uint256 p2pSupplyDelta, uint256 p2pBorrowDelta, uint256 p2pSupplyAmount, uint256 p2pBorrowAmount);
    function marketParameters(address) external view returns (uint16 reserveFactor, uint16 p2pIndexCursor);
    function marketPauseStatus(address) external view returns (bool isSupplyPaused, bool isBorrowPaused, bool isWithdrawPaused, bool isRepayPaused, bool isLiquidateCollateralPaused, bool isLiquidateBorrowPaused, bool isDeprecated);
    function p2pDisabled(address) external view returns (bool);
    function p2pSupplyIndex(address) external view returns (uint256);
    function p2pBorrowIndex(address) external view returns (uint256);
    function lastPoolIndexes(address) external view returns (uint32 lastUpdateBlockNumber, uint112 lastSupplyPoolIndex, uint112 lastBorrowPoolIndex);
    function marketStatus(address) external view returns (bool isCreated, bool isPaused, bool isPartiallyPaused);
    ...
    function setIncentivesVault(IIncentivesVault _newIncentivesVault) external;
    function setRewardsManager(IRewardsManager _rewardsManagerAddress) external;
    ...
    function setReserveFactor(address _poolToken, uint16 _newReserveFactor) external;
