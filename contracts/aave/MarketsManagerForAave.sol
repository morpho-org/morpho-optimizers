...
contract MarketsManagerForAave is Ownable {
    ...
    function _updateP2PExchangeRates(address _marketAddress) internal {
        ...
        if (positionsManager.supplyP2PDelta(_marketAddress) == 0) {
            ...
            .rayMul(WadRayMath.wadToRay(supplyP2PExchangeRate[_marketAddress]))
            .rayDiv(WadRayMath.wadToRay(normalizedIncome))
            .rayDiv(WadRayMath.wadToRay(positionsManager.supplyP2PAmount(_marketAddress))); // in RAY
        ...
        if (positionsManager.borrowP2PDelta(_marketAddress) == 0) {
            ...
            .rayMul(WadRayMath.wadToRay(borrowP2PExchangeRate[_marketAddress]))
            .rayDiv(WadRayMath.wadToRay(normalizedVariableDebt))
            .rayDiv(WadRayMath.wadToRay(positionsManager.borrowP2PAmount(_marketAddress))); // in RAY
