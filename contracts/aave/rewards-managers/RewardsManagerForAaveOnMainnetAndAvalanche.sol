...
contract RewardsManagerForAaveOnMainnetAndAvalanche is RewardsManagerForAave {
    ...
    function _getUpdatedIndex(address _asset, uint256 _totalBalance) internal override returns (uint256 newIndex) {
        ...
        if (blockTimestamp == lastTimestamp) return localData.lastIndex;
            ...
            (uint256 oldIndex, uint256 emissionPerSecond, uint256 lastTimestampOnAave) = aaveIncentivesController.getAssetData(_asset);

            newIndex = _getAssetIndex(oldIndex, emissionPerSecond, lastTimestampOnAave, _totalBalance);
    ...
    /// @inheritdoc RewardsManagerForAave
    function _getNewIndex(address _asset, uint256 _totalBalance) internal view override returns (uint256 newIndex) {
        LocalAssetData storage localData = localAssetData[_asset];
        uint256 blockTimestamp = block.timestamp;
        uint256 lastTimestamp = localData.lastUpdateTimestamp;
        if (blockTimestamp == lastTimestamp) return localData.lastIndex;
