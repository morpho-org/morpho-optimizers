...
contract RewardsManagerForAaveOnPolygon is RewardsManagerForAave {
    ...
    function _getUpdatedIndex(address _asset, uint256 _totalBalance) internal override returns (uint256 newIndex) {
        ...
        if (blockTimestamp == lastTimestamp) return localData.lastIndex;
            ...
            IAaveIncentivesController.AssetData memory assetData = aaveIncentivesController.assets(_asset);
            uint256 oldIndex = assetData.index;
            uint128 lastTimestampOnAave = assetData.lastUpdateTimestamp;

            newIndex = _getAssetIndex(oldIndex, assetData.emissionPerSecond, lastTimestampOnAave, _totalBalance);
