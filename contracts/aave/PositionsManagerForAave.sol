...
contract PositionsManagerForAave is PositionsManagerForAaveLogic {
    ...
    function supply(
        ...
        emit Supplied(msg.sender, _poolTokenAddress, _amount, supplyBalanceInOf[_poolTokenAddress][msg.sender].onPool, supplyBalanceInOf[_poolTokenAddress][msg.sender].inP2P, _referralCode);
    }
    ...
    function borrow(
        ...
        emit Borrowed(msg.sender, _poolTokenAddress, _amount, borrowBalanceInOf[_poolTokenAddress][msg.sender].onPool, borrowBalanceInOf[_poolTokenAddress][msg.sender].inP2P, _referralCode);
    }
    ...
    function withdraw(address _poolTokenAddress, uint256 _amount) external nonReentrant {
        ...
        uint256 toWithdraw = Math.min(_getUserSupplyBalanceInOf(_poolTokenAddress, msg.sender, IAToken(_poolTokenAddress).UNDERLYING_ASSET_ADDRESS()), _amount);
        ...
        emit Withdrawn(msg.sender, _poolTokenAddress, _amount, supplyBalanceInOf[_poolTokenAddress][msg.sender].onPool, supplyBalanceInOf[_poolTokenAddress][msg.sender].inP2P);
    }
    ...
    function repay(address _poolTokenAddress, uint256 _amount) external nonReentrant {
        ...
        uint256 toRepay = Math.min(_getUserBorrowBalanceInOf(_poolTokenAddress, msg.sender, IAToken(_poolTokenAddress).UNDERLYING_ASSET_ADDRESS()), _amount);

        _repay(_poolTokenAddress, msg.sender, toRepay, maxGas.repay);

        emit Repaid(msg.sender, _poolTokenAddress, _amount, borrowBalanceInOf[_poolTokenAddress][msg.sender].onPool, borrowBalanceInOf[_poolTokenAddress][msg.sender].inP2P);
    }
    ...
    function liquidate(
        ...
        (vars.debtValue, , vars.liquidationValue) = _getUserHypotheticalBalanceStates(_borrower, address(0), 0, 0);
        ...
        vars.borrowBalance = _getUserBorrowBalanceInOf(_poolTokenBorrowedAddress, _borrower, vars.tokenBorrowedAddress);

        if (_amount > (vars.borrowBalance * LIQUIDATION_CLOSE_FACTOR_PERCENT) / MAX_BASIS_POINTS) revert AmountAboveWhatAllowedToRepay(); // Same mechanism as Aave. Liquidator cannot repay more than part of the debt (cf close factor on Aave).
        ...
        (, , vars.liquidationBonus, vars.collateralReserveDecimals, ) = lendingPool.getConfiguration(vars.tokenCollateralAddress).getParamsMemory();
        (, , , vars.borrowedReserveDecimals, ) = lendingPool.getConfiguration(vars.tokenBorrowedAddress).getParamsMemory();
        ...
        vars.amountToSeize = (_amount * vars.borrowedPrice * vars.collateralTokenUnit * vars.liquidationBonus) / (vars.borrowedTokenUnit * vars.collateralPrice * MAX_BASIS_POINTS); // Same mechanism as aave. The collateral amount to seize is given.

        vars.supplyBalance = _getUserSupplyBalanceInOf(_poolTokenCollateralAddress, _borrower, vars.tokenCollateralAddress);
        ...
        emit Liquidated(msg.sender, _borrower, _amount, _poolTokenBorrowedAddress, vars.amountToSeize, _poolTokenCollateralAddress);
    }
    ...
    function claimToTreasury(address _poolTokenAddress) external onlyOwner isMarketCreatedAndNotPaused(_poolTokenAddress) {
    ...
    function claimRewards(address[] calldata _assets, bool _swap) external nonReentrant {
        ...
        if (amountToClaim == 0) revert AmountIsZero();
            ...
            if (_swap) {
                ...
                uint256 amountClaimed = aaveIncentivesController.claimRewards(_assets, amountToClaim, swapManager);
                uint256 amountOut = ISwapManager(swapManager).swapToMorphoToken(amountClaimed, msg.sender);
                emit RewardsClaimedAndSwapped(msg.sender, amountClaimed, amountOut);
            } else {
                uint256 amountClaimed = aaveIncentivesController.claimRewards(_assets, amountToClaim, msg.sender);
