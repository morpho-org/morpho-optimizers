...
contract PositionsManagerForAave is PositionsManagerForAaveStorage {
    ...
    function claimRewards(address[] calldata _assets) external {
        uint256 amountToClaim = _accrueRewardsForAssets(_assets, type(uint256).max, msg.sender);
        if (amountToClaim > 0) {
            userUnclaimedRewards[msg.sender] = userUnclaimedRewards[msg.sender] - amountToClaim;
    ...
    function _accrueRewardsForAssets(
        ...
        for (uint256 i = 0; i < _assets.length; i++) {
            ...
            userState[i].stakedByUser = variableDebtTokenAddress == _assets[i]
                ? borrowBalanceInOf[aTokenAddress][_user].onPool
                : supplyBalanceInOf[aTokenAddress][_user].onPool;
        ...
        return _amount > unclaimedRewards ? unclaimedRewards : _amount;
    }
    ...
    function _updateUserAssetAndAccruedRewards(
        address _user,
        address _asset,
