...
contract Core {
	...
	function lend(uint256 _amount, address _token, address _module) external {
		require(tokenToLendAllowed[_token], "");
		(bool success, bytes memory result) = _module.delegatecall(abi.encodeWithSignature("lend(uint256,address)", _amount, _token));
		require(success, "");
	}

	function borrow(uint256 _amount, address _token, address _module) external {
		require(tokenToLendAllowed[_token], "");
		(bool success, bytes memory result) = _module.delegatecall(abi.encodeWithSignature("borrow(uint256,address)", _amount, _token));
		require(success, "");
	}

	function payback(uint256 _amount, address _token, address _module) external {
		require(tokenToLendAllowed[_token], "");
		(bool success, bytes memory result) = _module.delegatecall(abi.encodeWithSignature("payback(uint256,address)", _amount, _token));
		require(success, "");
	}

	function cashOut(uint256 _amount, address _token, address _module) external {
		require(tokenToLendAllowed[_token], "");
		(bool success, bytes memory result) = _module.delegatecall(abi.encodeWithSignature("cashOut(uint256,address)", _amount, _token));
		require(success, "");
	}

	function cashOutAll(address _token, address _module) external {
		require(tokenToLendAllowed[_token], "");
		(bool success, bytes memory result) = _module.delegatecall(abi.encodeWithSignature("cashOutAll(address)", _token));
		require(success, "");
	}
