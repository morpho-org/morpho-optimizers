...
contract CompoundModuleMultiAsset {
    ...
    function borrow(uint256 _amount, address _token) external {
        ...
        uint256 unusedCollateral = collateralBalanceOf[_token][msg.sender]
        .total - collateralBalanceOf[_token][msg.sender].used;
        ...
        uint256 availableTokenToBorrow = token.balanceOf(address(this)) *
        ...
        // TODO: check that
        collateralBalanceOf[_token][msg.sender].used += collateralBalanceOf[
            _token
        ][msg.sender]
        .usable;
    ...
    function paybackAll(uint256 _amount, address _token) external {
        require(tokenAllowed[_token], "");
        require(
            _amount >= borrowingBalanceOf[_token][msg.sender],
            "need to payback all"
        );
        IERC20(_token).transferFrom(msg.sender, address(this), _amount);
    ...
    function redeemCollateral(uint256 _amount, address _token) external {
        require(
            isNotBorrowing(msg.sender),
            "You have to repay your borrowing to redeem collateral."
        );
        address cToken = tokenToTokenModule[_token][
            moduleAddress[keccak256("Compound")]
        ];
        require(cToken != "0x0", "");
        require(_amount > 0, "Amount cannot be 0");
        ...
        uint256 amountRedeemed = amountToRedeem *
            ICErc20(cToken).exchangeRateCurrent();
    ...
    function repayAll(uint256 _amount, address _token) external {
        ...
        borrowingOperations[msg.sender].amountBorrowed -=
            msg.value /
            COLLATERAL_FACTOR;
        borrowingOperations[msg.sender].amountRepaid +=
            msg.value /
            COLLATERAL_FACTOR;
    ...
    function _supplyEthToCompound(address _cToken)
        internal
        payable
        returns (bool)
    {
        uint256 result = CEth(_cToken).mint{value: msg.value}();
        require(result == 0, "");
    }

    function _supplyErc20ToCompound(address _token, uint256 _amount)
        internal
        returns (uint256)
    {
        ...
        ICErc20 cToken = ICErc20(
            tokenToTokenModule[_token][moduleAddress[keccak256("Compound")]]
        );
        // Approve transfer on the ERC20 contract.
        underlying.approve(address(cToken), _amount);
        // Mint cTokens.
        uint256 result = cToken.mint(_amount);
        return result;
    }
    ...
    function _redeemCEth(uint256 _amount, bool _redeemType)
        internal
        payable
        returns (bool)
    {
        ...
        ICEth cToken = ICEth(
            tokenToTokenModule["0x0"][moduleAddress[keccak256("Compound")]]
        );
    ...
    // TODO: can be public
    function _calculateAmountToRepay(address _borrower, address _token)
        internal
        returns (uint256)
    {
        uint256 amountToRepay = borrowingOperations[msg.sender].amountBorrowed -
            borrowingOperations[msg.sender].amountRepaid;
    ...
    function _findUnusedCTokensAndUse(uint256 _amount, address _token)
        internal
    {
        ...
        while (remainingLiquidityToUse > 0 && i < list.length) {
            ...
            uint256 usable = lendingBalanceOf[_token][lenderAddress].total -
                lendingBalanceOf[_token][lenderAddress].used;
            if (usable > 0) {
                // We increase used balance of this lender by eihter the max the lender can use or the max we need to use
                lendingBalanceOf[_token][lenderAddress].used += min(
                    usable,
                    remainingLiquidityToUse
                );
    ...
    function _findUsedCTokensAndUnunse(uint256 _amount, address _token)
        internal
    {
