...
contract RewardsManager is IRewardsManager, Ownable {
    ...
    function claimRewards(address[] calldata _cTokenAddresses, address _user) external onlyMorpho returns (uint256 totalUnclaimedRewards) {
    ...
    /// @param _user The address of the user.
    function getUserUnclaimedRewards(address[] calldata _cTokenAddresses, address _user) external view returns (uint256 unclaimedRewards) {
        ...
        for (uint256 i; i < _cTokenAddresses.length; i++) {
            ...
            unclaimedRewards += getAccruedSupplierComp(_user, cTokenAddress, morpho.supplyBalanceInOf(cTokenAddress, _user).onPool);
            unclaimedRewards += getAccruedBorrowerComp(_user, cTokenAddress, morpho.borrowBalanceInOf(cTokenAddress, _user).onPool);
        }
    }
    ...
    function getLocalCompSupplyState(address _cTokenAddress) external view override returns (IComptroller.CompMarketState memory) {
    ...
    function getLocalCompBorrowState(address _cTokenAddress) external view override returns (IComptroller.CompMarketState memory) {
    ...
    function accrueUserUnclaimedRewards(address[] calldata _cTokenAddresses, address _user) public returns (uint256 unclaimedRewards) {
        unclaimedRewards = userUnclaimedCompRewards[_user];
        ...
        for (uint256 i; i < _cTokenAddresses.length; i++) {
            ...
            unclaimedRewards += _accrueSupplierComp(_user, cTokenAddress, morpho.supplyBalanceInOf(cTokenAddress, _user).onPool);

            _updateBorrowIndex(cTokenAddress);
            unclaimedRewards += _accrueBorrowerComp(_user, cTokenAddress, morpho.borrowBalanceInOf(cTokenAddress, _user).onPool);
        }
    ...
    function getUpdatedSupplyIndex(address _cTokenAddress) public view returns (uint256) {
        ...
        if (localSupplyState.block == blockNumber) return localSupplyState.index;
            ...
            IComptroller.CompMarketState memory supplyState = comptroller.compSupplyState(_cTokenAddress);

            if (supplyState.block == blockNumber) return supplyState.index;
            else {
                uint256 deltaBlocks = localSupplyState.block == 0 ? blockNumber - supplyState.block : blockNumber - localSupplyState.block;
                ...
                if (supplySpeed > 0) {
                    ...
                    uint256 formerIndex = localSupplyState.index == 0 ? supplyState.index : localSupplyState.index;
    ...
    function getUpdatedBorrowIndex(address _cTokenAddress) public view returns (uint256) {
        ...
        if (localBorrowState.block == blockNumber) return localBorrowState.index;
            ...
            IComptroller.CompMarketState memory borrowState = comptroller.compBorrowState(_cTokenAddress);

            if (borrowState.block == blockNumber) return borrowState.index;
            else {
                uint256 deltaBlocks = localBorrowState.block == 0 ? blockNumber - borrowState.block : blockNumber - localBorrowState.block;
                uint256 borrowSpeed = comptroller.compBorrowSpeeds(_cTokenAddress);

                if (borrowSpeed > 0) {
                    uint256 borrowAmount = ICToken(_cTokenAddress).totalBorrows().div(ICToken(_cTokenAddress).borrowIndex());
                    uint256 compAccrued = deltaBlocks * borrowSpeed;
                    uint256 ratio = borrowAmount > 0 ? (compAccrued * 1e36) / borrowAmount : 0;
                    uint256 formerIndex = localBorrowState.index == 0 ? borrowState.index : localBorrowState.index;
                    return formerIndex + ratio;
    ...
    function _updateSupplyIndex(address _cTokenAddress) internal {
        IComptroller.CompMarketState storage localSupplyState = localCompSupplyState[_cTokenAddress];
        uint256 blockNumber = block.number;
        ...
        if (localSupplyState.block == blockNumber) return;
            ...
            if (supplyState.block == blockNumber) {
                ...
                if (supplySpeed > 0) {
                    ...
                    localCompSupplyState[_cTokenAddress] = IComptroller.CompMarketState({index: CompoundMath.safe224(index), block: CompoundMath.safe32(blockNumber)});
    ...
    function _updateBorrowIndex(address _cTokenAddress) internal {
        IComptroller.CompMarketState storage localBorrowState = localCompBorrowState[_cTokenAddress];
        uint256 blockNumber = block.number;
