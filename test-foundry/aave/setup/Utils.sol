...
contract Utils is DSTest {
    ...
    function computeTheoreticalRates(
        uint256 _poolSupplyRate,
        uint256 _poolBorrowRate,
        uint256 _reserveFactor
    ) public pure returns (uint256 p2pSupplyRate_, uint256 p2pBorrowRate_) {
        // rate = 2/3 * poolSupplyRate + 1/3 * poolBorrowRate.
        uint256 rate = (2 * _poolSupplyRate + _poolBorrowRate) / 3;

        p2pSupplyRate_ = rate - (_reserveFactor * (rate - _poolSupplyRate)) / 10_000;
        p2pBorrowRate_ = rate + (_reserveFactor * (_poolBorrowRate - rate)) / 10_000;
    }
}
