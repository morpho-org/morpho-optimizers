...
contract TestRatesLens is TestSetup {
    ...
    function testGetRatesPerYear() public {
        ...
        assertApproxEqAbs(p2pSupplyRate, expectedP2PSupplyRate, 1, "p2p supply rate");
        assertApproxEqAbs(p2pBorrowRate, expectedP2PBorrowRate, 1, "p2p borrow rate");
        assertEq(poolSupplyRate, expectedPoolSupplyRate, "pool supply rate");
        assertEq(poolBorrowRate, expectedPoolBorrowRate, "pool borrow rate");
    }
    ...
    function testUserSupplyBorrowRatesShouldEqualP2PRatesWhenFullyMatched() public {
        ...
        // Approx because of dust pool balance
        assertApproxEqAbs(
            supplyRatePerYear,
            p2pSupplyRate,
            p2pSupplyRate / 1e8,
            "unexpected supply rate"
        );
    ...
    function testUserSupplyRateShouldEqualMidrateWhenHalfMatched() public {
        ...
        (uint256 balanceInP2P, uint256 balanceOnPool, uint256 totalBalance) = lens
        .getCurrentSupplyBalanceInOf(aDai, address(supplier1));
        assertApproxEqAbs(balanceInP2P, balanceOnPool, amount / 1e8, "pool and p2p balances");
        assertApproxEqAbs(
            supplyRatePerYear,
            (p2pSupplyRate.wadMul(balanceInP2P.wadDiv(totalBalance)) +
                poolSupplyRate.wadMul(balanceOnPool.wadDiv(totalBalance))),
            1,
    ...
    function testSupplyRateShouldEqualPoolRateWithFullSupplyDelta() public {
        ...
        assertApproxEqAbs(p2pSupplyRate, poolSupplyRate, p2pSupplyRate / 1e18);
    }
    ...
    function testNextSupplyRateShouldEqualP2PRateWhenDoubleSupply() public {
        ...
        assertApproxEqAbs(
            supplyRatePerYear,
            p2pSupplyRatePerYear,
            p2pSupplyRatePerYear / 1e8,
            "unexpected supply rate per block"
        );
        assertApproxEqAbs(balanceOnPool, 0, amount / 1e8, "unexpected pool balance");
        assertApproxEqAbs(balanceInP2P, expectedBalanceInP2P, 1, "unexpected p2p balance");
        assertApproxEqAbs(
            totalBalance,
            expectedBalanceInP2P,
            amount / 1e8,
            "unexpected total balance"
        );
    }
    ...
    function testNextSupplyRateShouldEqualMidrateWithHalfBorrowDeltaAndNoBorrowerOnPool() public {
        ...
        assertApproxEqAbs(
            supplyRatePerYear,
            (p2pSupplyRatePerYear.wadMul(balanceInP2P.wadDiv(totalBalance)) +
                poolSupplyRatePerYear.wadMul(balanceOnPool.wadDiv(totalBalance))),
            1,
            "unexpected supply rate per block"
        );
        assertApproxEqAbs(
            ...
            expectedBalanceOnPool / 1e8,
            "unexpected pool balance"
        );
        assertApproxEqAbs(
            balanceInP2P,
            expectedBalanceInP2P,
            expectedBalanceInP2P / 1e8,
            "unexpected p2p balance"
        );
        assertEq(
            totalBalance,
            expectedBalanceOnPool + expectedBalanceInP2P,
    ...
    function testNextBorrowRateShouldEqualMidrateWithHalfSupplyDeltaAndNoSupplierOnPool() public {
        ...
        assertApproxEqAbs(
            borrowRatePerYear,
            (p2pBorrowRatePerYear.wadMul(balanceInP2P.wadDiv(totalBalance)) +
                poolBorrowRatePerYear.wadMul(balanceOnPool.wadDiv(totalBalance))),
            1,
            "unexpected borrow rate per block"
        );
        assertApproxEqAbs(
    ...
    function testAverageRatesShouldEqualP2PRatesWhenFullyMatched() public {
        ...
        // Approx because there is some extra dust supply on pool
        assertApproxEqAbs(
            supplyRatePerYear,
            p2pSupplyRate,
            p2pSupplyRate / 1e8,
        ...
        assertApproxEqAbs(borrowRatePerYear, p2pBorrowRate, 1, "unexpected borrow rate");
        // Approx because there is some extra dust supply on pool
        assertApproxEqAbs(poolSupplyAmount, 0, amount / 1e8, "pool supply and borrow");
        assertApproxEqAbs(poolBorrowAmount, 0, 1, "pool borrow");
        assertEq(p2pSupplyAmount, p2pBorrowAmount, "p2p supply and borrow");
        assertApproxEqAbs(p2pBorrowAmount, amount, 1, "p2p borrow");
    }
    ...
    function testAverageSupplyRateShouldEqualMidrateWhenHalfMatched() public {
        ...
        assertApproxEqAbs(
            supplyRatePerYear,
            (p2pSupplyRate.wadMul(p2pSupplyAmount.wadDiv(totalSupply)) +
                poolSupplyRate.wadMul(poolSupplyAmount.wadDiv(totalSupply))),
            1
        );
    }
    ...
    function testAverageBorrowRateShouldEqualMidrateWhenHalfMatched() public {
        ...
        assertApproxEqAbs(
            borrowRatePerYear,
            (p2pBorrowRate.wadMul(p2pBorrowAmount.wadDiv(totalBorrow)) +
                poolBorrowRate.wadMul(poolBorrowAmount.wadDiv(totalBorrow))),
            1
        );
    }
