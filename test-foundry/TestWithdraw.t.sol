...
contract TestWithdraw is TestSetup {
    ...
    function test_withdraw_3_3_6() public {
        ...
        // supplier1 and 20 borrowers are matched for suppliedAmount
        supplier1.approve(dai, suppliedAmount);
        supplier1.supply(aDai, suppliedAmount); // Rates updated
        ...
        for (uint256 i = 0; i < 20; i++) {
            borrowers[i].approve(usdc, to6Decimals(collateral));
            borrowers[i].supply(aUsdc, to6Decimals(collateral)); // Rates updated
            borrowers[i].borrow(aDai, borrowedAmount, type(uint64).max); // Rates updated
        }
