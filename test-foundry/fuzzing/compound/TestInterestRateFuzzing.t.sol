...
contract TestInterestRateFuzzing is InterestRatesManager, Test {
    ...
    function testFuzzInterestRates(
        ...
        hevm.assume(
            (_poolBorrowIndex * WAD) / _lastPoolBorrowIndex >
                (_poolSupplyIndex * WAD) / _lastPoolSupplyIndex
        );
        hevm.assume(
            (_p2pSupplyAmount * _p2pSupplyIndex) / WAD > (_p2pSupplyDelta * _poolSupplyIndex) / WAD
        );
        hevm.assume(
            (_p2pBorrowAmount * _p2pBorrowIndex) / WAD > (_p2pBorrowDelta * _poolBorrowIndex) / WAD
        );
